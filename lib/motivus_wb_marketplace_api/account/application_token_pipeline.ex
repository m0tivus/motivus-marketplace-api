defmodule MotivusWbMarketplaceApi.Account.ApplicationTokenPipeline do
  @moduledoc """
  Handles whitelisting of routes using application_tokens.
  Relies on 'mwbat' token implementation for Guardian, see MotivusWbMarketplaceApi.Account.JwtOrMwbt 
  """
  import Plug.Conn
  ...
  defp get_path_info(conn) do
    ...
    %{plug: controller, plug_opts: action} =
      Phoenix.Router.route_info(MotivusWbMarketplaceApiWeb.Router, method, request_path, host)

    Phoenix.Router.routes(MotivusWbMarketplaceApiWeb.Router)
